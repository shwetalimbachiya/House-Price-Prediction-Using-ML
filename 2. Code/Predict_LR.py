...
def plot_features(data, feature):
    ...
    plt.title('SalePrice in function of LotArea using Linear Regression')
    plt.show()


def plot_features_default(data, feature):
    plt.plot(data.values, feature, 'bo')
    plt.ylabel('SalePrice')
    plt.xlabel('LotArea - Lot size in square feet')
    plt.title('Actual SalePrice given in dataset')
    plt.show()
...
def main():
    ...
    # Visualizing given dataset by plotting graph
    plot_features_default(test['LotArea'], y_test)

    # Visualizing model by plotting graph
    plot_features(test['LotArea'], y_preds)

    # Generating output csv file
    output.to_csv('LR_prediction.csv', index=False)
